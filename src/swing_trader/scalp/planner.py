...
from .strategy import (V1_K, V1_STOP, V2_STOP, V3_STOP, V3_TARGET,
                       V4_STOP, V4_TARGET, V5_STOP, V5_TARGET, PlanItem)
...
def _items(model: str, cands: list[dict], cash: float, quotes: dict,
    ...
    for c in cands:
        ...
        if model == "v1":
            ...
            out.append(PlanItem(model="v5", ticker=c["ticker"], name=c["name"], qty=q,
...
def build_plan(candidates: list[dict], cash_by_model: dict, scenario: dict,
    ...
    # v5 상따 모멘텀 — 폭등+대량거래+신고가 자체가 강한 필터라 uptrend 불요.
    # '오직 대장주만' 원칙: 전일 등락률 내림차순(가장 강한 종목 우선) + 상한 2.
    chg = lambda c: c.get("prev_chg_pct") or 0.0   # noqa: E731
    v5c = sorted([c for c in ranked if c.get("v5_ok")], key=chg, reverse=True)
    v5c_base = sorted([c for c in base if c.get("v5_ok")], key=chg, reverse=True)
