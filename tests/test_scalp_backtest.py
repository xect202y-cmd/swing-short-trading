...
def test_simulate_produces_trades_with_dates():
    ...
    assert set(trades) == {"v1", "v2", "v3", "v4"}
    all_t = trades["v1"] + trades["v2"] + trades["v3"] + trades["v4"]
...
def test_liquidity_filter_blocks_all():
    ...
    assert trades == {"v1": [], "v2": [], "v3": [], "v4": []}


def test_needs_61_bars():
    assert simulate_stock("005930", _df(n=50), min_tv_eok=0) == {"v1": [], "v2": [], "v3": [], "v4": []}
